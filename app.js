...
// const reviewSchema= require("./mongoSchema")
...
//connect
// mongoose.connect("mongodb://0.0.0.0:27017/Ngopics", {useNewUrlParser: true, useUnifiedTopology: true});
// app.set("views", path.join(__dirname,"views"))
// app.use(session({
...
app.get("/", async function (req, res) {
  ...
  if (obj != null) {
    res.status(200).render('landingPageIndex', {
      ngos: obj
    });
  } else {
    ...
    if (obj != null) {
      if (arrofngo.length > 0) {
        res.status(200).render('User', {
          ...
          useritem: userobj,
          reviews: review,
          arrofngo: reportschemaobj.ngoname
        });
      } else {
        res.status(200).render('User', {
          name: req.session.user.username,
...
app.post("/reportdetails", async (req, res) => {
  ...
  await ngoschema.deleteMany({
      username: req.body.checkbox
    })
  ...
  await ngoschemaregist.deleteOne({
      username: req.body.checkbox
    })
    .catch(function (err) {
      console.log(err);
  ...
  //   // { new: true }
  //  )
  //   await reportschema.find(
  //     { ngoname: { $elemMatch: {ngoname:req.body.checkbox}} }
  //  ).updateOne( 
  //     { $pull: { ngoname:  req.body.checkbox } }
  //    )
...
app.post("/reportdata", async (req, res) => {
  ...
  const check_username_exists = await reportschema.findOne({
    username: username
  });
...
app.post('/getrating', async (req, res) => {
  ...
  const rating = await reviewschema.find({
    ngoname: req.body.payload
  });
  const obj = {
    payload: rating,
